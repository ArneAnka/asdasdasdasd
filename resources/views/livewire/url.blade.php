<div>
    <div class="">
        ...
        <div>
            Inlagd {{ $collection->created_at }} ({{ $collection->created_at->diffForHumans() }}), av <span class="underline">{{ $collection->user->nickname }}</span>
        </div>
    </div>
    <div class="mt-0 flex items-center">
        ...
        <div class="flex mr-2 text-gray-700 text-sm mr-3">
            <a href="{{route('url.show', $collection)}}" class="inline-flex items-center">
                <svg fill="none" viewBox="0 0 24 24" class="w-4 h-4 mr-1" stroke="currentColor">
        ...
        <div class="flex mr-2 text-gray-700 text-sm mr-8">
            <span>
                <a class="underline" href="#/" x-on:click="confirmUrlsDeletion" x-data="{
                            confirmUrlsDeletion () {
                                if(window.confirm('Säker?')){
                                    @this.call('deleteUrls')
                            }
                        }
                    }">Radera url</a>
            </span>
